...
################################################
docker build ./docker -t apachepulsar/pulsar-aop

################################################
# Start AoP enabled Pulsar in standalone mode
################################################
docker run --name pulsar -dt -p 5672:5672  -p 6650:6650  -p 8080:8080 \
  --mount source=pulsardata,target=/pulsar/data \
  --mount source=pulsarconf,target=/pulsar/conf \
  apachepulsar/pulsar-aop bin/pulsar standalone
